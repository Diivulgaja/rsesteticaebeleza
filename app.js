const App = (() => {
            ...
            const STORAGE_KEYS = {
                name: 'rosouza_name',
                phone: 'rosouza_phone',
                bookingProgress: 'rosouza_booking_progress'
            };
        ...
        const fallbackGoogleReviews = [
            ...
            function syncPublicServiceCatalog() {
                window.__RS_FALLBACK_SERVICES__ = fallbackServices.map((service) => ({
                    id: service.id,
                    title: service.title,
                    category: service.category || '',
                    duration_minutes: Number.parseInt(String(service.duration || '').replace(/\D/g, ''), 10) || 0
                }));

                window.__RS_SERVICE_CATALOG__ = services.map((service) => ({
                    id: service.id,
                    title: service.title,
                    category: service.category || '',
                    duration_minutes: Number.parseInt(String(service.duration || '').replace(/\D/g, ''), 10) || 0
            ...
            function assertSupabaseConfig() {
                ...
                console.error('Supabase não configurado. Verifique se app.config.js foi publicado e carregado antes do app.js.', {
                    hasAppConfig: Boolean(window.APP_CONFIG),
                    hasAltConfig: Boolean(window.CONFIG || window.__APP_CONFIG__ || window.__RS_RUNTIME_CONFIG__),
                    currentUrl: window.location.href
                });
                alert('Configuração do Supabase ausente. Verifique se o arquivo app.config.js foi publicado e se está carregando antes do app.js.');
                return false;
            }
            ...
            function hasAuthHashTokens(hash) {
                const value = String(hash || '').replace(/^#/, '');
                if (!value) return false;
                const params = new URLSearchParams(value);
                return ['access_token', 'refresh_token', 'expires_at', 'expires_in', 'token_type', 'provider_token', 'provider_refresh_token', 'type'].some((key) => params.has(key));
            }
            ...
            function cleanupAuthHash() {
                ...
                if (!hasAuthHashTokens(window.location.hash)) {
                    return;
                }
            ...
            function createSupabaseClient() {
                ...
                if (window.__RS_SUPABASE_CLIENT__) {
            ...
            function getPublishedReviewsAverage() {
                if (!Array.isArray(googleReviews) || googleReviews.length === 0) {
                    return '0.0';
                }

                const total = googleReviews.reduce((sum, review) => sum + (Number(review.rating) || 0), 0);
                return (total / googleReviews.length).toFixed(1);
            }

            function updatePublishedReviewsSummary() {
                const averageTargets = [
                    document.getElementById('reviews-summary-average'),
                    document.getElementById('specialist-reviews-average')
                ].filter(Boolean);
                const countTarget = document.getElementById('reviews-summary-count');
                const average = getPublishedReviewsAverage();
                const count = Array.isArray(googleReviews) ? googleReviews.length : 0;

                averageTargets.forEach((element) => {
                    element.innerHTML = `${average} <i data-lucide="star" class="w-5 h-5 text-yellow-400 fill-current"></i>`;
                });
            ...
            async function loadReviewsFromSupabase() {
                ...
                googleReviews = data.map((item) => normalizeReviewRecord({
                    ...item,
                    public_name: item.customer_name || item.public_name || 'Cliente',
            ...
            async function fetchAvailableTimeSlots(date) {
                ...
                if (!isRecoverableRpcError(response.error)) {
                    throw response.error;
                }

                console.warn('list_available_slots indisponível/ambígua, usando consulta direta.', response.error);
                return fetchAvailableTimeSlotsDirect(date);
            }

            async function fetchAvailableTimeSlotsDirect(date) {
                const bookingDate = formatDateToIso(date);
                const weekday = date.getDay();

                const [serviceRes, hoursRes, appointmentsRes] = await Promise.all([
                    supabase
                        .from('services')
                ...
                if (serviceRes.error && !isMissingDbObjectError(serviceRes.error)) {
                    throw serviceRes.error;
                }
                if (hoursRes.error && !isMissingDbObjectError(hoursRes.error)) {
                    throw hoursRes.error;
                }
                if (appointmentsRes.error && !isMissingDbObjectError(appointmentsRes.error)) {
                    throw appointmentsRes.error;
                }

                const durationMinutes = Number(serviceRes.data?.duration_minutes || getServiceDurationInMinutes(state.selectedService) || 60);
                const isActive = hoursRes.data?.is_active;
                if (isActive === false) {
                    return [];
                }

                const startHour = String(hoursRes.data?.start_time || '09:00').slice(0, 5);
                const endHour = String(hoursRes.data?.end_time || '18:00').slice(0, 5);
                const busyRanges = (appointmentsRes.data || [])
                    .filter((item) => !['cancelled', 'expired'].includes(String(item?.status || '').toLowerCase()))
                    .map((item) => ({
                        start: item?.start_at ? new Date(item.start_at).getTime() : null,
                        end: item?.end_at ? new Date(item.end_at).getTime() : null
                    }))
                ...
                cursor.setHours(startHourValue || 9, startMinuteValue || 0, 0, 0);
                const endBoundary = new Date(date);
                endBoundary.setHours(endHourValue || 18, endMinuteValue || 0, 0, 0);

                while (cursor.getTime() < endBoundary.getTime()) {
                    const slotStart = cursor.getTime();
                    const slotEnd = slotStart + durationMinutes * 60 * 1000;
                    const overlaps = busyRanges.some((item) => slotStart < item.end && slotEnd > item.start);
                    const slotLabel = `${String(cursor.getHours()).padStart(2, '0')}:${String(cursor.getMinutes()).padStart(2, '0')}`;

                    if (!overlaps && !isPastTimeSlot(date, slotLabel)) {
            ...
            function isRecoverableRpcError(error) {
                ...
                return (
                    ...
                    message.includes('function overloading can be resolved') ||
                    message.includes('multiple choices') ||
                    message.includes('ambiguous')
                );
            }

            function isBookingConflictError(error) {
                const message = [error?.message, error?.details, error?.hint, error?.code]
                    .filter(Boolean)
                    .join(' ')
                    .toLowerCase();
            ...
            function isBookingSchemaInsertError(error) {
                const message = [error?.message, error?.details, error?.hint, error?.code]
                    .filter(Boolean)
                    .join(' ')
                    .toLowerCase();
                return (
                    ...
                    message.includes("null value in column 'id'") ||
                    message.includes('appointments violates not-null constraint') ||
            ...
            async function ensureCustomerProfileForBooking(payload) {
                ...
                try {
                    ...
                    const profilePayload = {
                        p_full_name: sanitizeRpcText(payload.customerName || meta.full_name || meta.name || user.email || 'Cliente'),
                        p_email: sanitizeRpcText(user.email || meta.email || ''),
                        p_phone: sanitizeRpcText(payload.customerPhone || ''),
                        p_avatar_url: sanitizeRpcText(meta.avatar_url || ''),
                        p_auth_provider: 'google'
                    };

                    const profileResponse = await supabase.rpc('upsert_my_customer_profile', profilePayload);
                    if (!profileResponse.error) {
                        return profileResponse.data || null;
                    }
                } catch (error) {
                    console.warn('Não foi possível sincronizar o perfil antes do agendamento.', error);
                }

                return null;
            }

            async function createBookingRecordFallback(payload) {
                if (!supabase) {
                ...
                const user = sessionData?.session?.user || null;
                if (!user) {
                    throw new Error('Faça login com Google para concluir o agendamento.');
                }

                const customerProfile = await ensureCustomerProfileForBooking(payload);
                const durationMinutes = getServiceDurationInMinutes(state.selectedService) || 60;
                const [hours, minutes] = String(payload.slotTime || '').split(':').map(Number);
                const startDate = new Date(state.selectedDate.fullDate);
                startDate.setHours(hours || 0, minutes || 0, 0, 0);
                const endDate = new Date(startDate);
                endDate.setMinutes(endDate.getMinutes() + durationMinutes);
                const bookingReference = `RS${Date.now().toString(36).slice(-6).toUpperCase()}`;

                let conflictProbe = await supabase
                    .from('public_booking_availability')
                    .select('id, status, start_at, end_at')
                    .eq('booking_date', payload.bookingDate)
                    .order('start_at', { ascending: true });

                if (conflictProbe.error) {
                    conflictProbe = await supabase
                        .from('appointments')
                        .select('id, status, start_at, end_at')
                        .eq('booking_date', payload.bookingDate)
                        .order('start_at', { ascending: true });
                ...
                if (!conflictProbe.error && Array.isArray(conflictProbe.data)) {
                    const hasConflict = conflictProbe.data.some((item) => {
                        if (!item?.start_at || !item?.end_at) return false;
                        if (['cancelled', 'expired'].includes(String(item.status || '').toLowerCase())) return false;
                        const currentStart = new Date(item.start_at).getTime();
                        const currentEnd = new Date(item.end_at).getTime();
                        return startDate.getTime() < currentEnd && endDate.getTime() > currentStart;
                    });
                ...
                const baseInsert = {
                    ...
                    price: Number(state.selectedService?.price || 0) || 0,
                    quoted_price: Number(state.selectedService?.price || 0) || 0,
                    final_price: Number(state.selectedService?.price || 0) || 0,
                ...
                for (const variant of variants) {
                    ...
                    if (!response.error) {
                        return {
                            success: true,
                            appointment_id: response.data?.id || null,
                            booking_reference: response.data?.booking_reference || bookingReference,
                            status: response.data?.status || 'pending'
                        };
                    }

                    lastError = response.error;
                    if (isBookingConflictError(lastError)) {
                        return {
                            success: false,
                            code: 'TIME_SLOT_UNAVAILABLE',
                            message: 'Esse horário acabou de ser reservado por outra cliente. Escolha um novo horário.'
                ...
                throw lastError || new Error('Não foi possível registrar o agendamento.');
            }
            ...
            async function createBookingRecord(payload) {
                ...
                if (isRecoverableRpcError(response.error) || response.error?.code === 'PGRST202' || isBookingSchemaInsertError(response.error)) {
                    console.warn('create_public_booking indisponível/incompatível, usando inserção direta.', response.error);
                    return createBookingRecordFallback(payload);
                }
            ...
            function cacheDomElements() {
                Object.assign(dom, {
                    ...
                    bookingLoginNotice: document.getElementById('booking-login-notice'),
                    bookingLoginButton: document.getElementById('booking-login-button'),
            ...
            async function initialize() {
                ...
                await restoreBookingProgress();
                bindEvents();
                await syncBookingAuthState();
                cleanupAuthHash();
                initScrollObserver();
                refreshIcons();
            }
            ...
            function bindEvents() {
                ...
                if (dom.bookingLoginButton) {
                    dom.bookingLoginButton.addEventListener('click', loginForBooking);
                }

                if (supabase && typeof supabase.auth?.onAuthStateChange === 'function') {
                    supabase.auth.onAuthStateChange(async (_event, session) => {
                        const user = session?.user || null;
                        await syncBookingAuthState(user);
                        cleanupAuthHash();
                    });
                }
            }
            ...
            async function getAuthenticatedUser() {
                ...
                if (error) {
                    console.error('Erro ao verificar sessão da cliente:', error);
                    return null;
                }
            ...
            function getBookingProgressSnapshot() {
                return {
                    step: state.step,
                    activeCategory: state.activeCategory,
                    selectedServiceId: state.selectedService?.id || null,
                    selectedDateIso: state.selectedDate ? formatDateToIso(state.selectedDate.fullDate) : null,
                    selectedTime: state.selectedTime || null
                };
            }

            function persistBookingProgress() {
                try {
                    localStorage.setItem(STORAGE_KEYS.bookingProgress, JSON.stringify(getBookingProgressSnapshot()));
                } catch (error) {
                    console.warn('Não foi possível salvar o progresso do agendamento.', error);
                }
            }

            function clearBookingProgress() {
                try {
                    localStorage.removeItem(STORAGE_KEYS.bookingProgress);
                } catch (error) {
                    console.warn('Não foi possível limpar o progresso do agendamento.', error);
                }
            }

            async function syncBookingAuthState(forcedUser = undefined) {
                const user = forcedUser === undefined ? await getAuthenticatedUser() : forcedUser;
                const isLoggedIn = Boolean(user);

                if (dom.bookingLoginNotice) {
                    dom.bookingLoginNotice.classList.toggle('hidden', isLoggedIn);
                }

                if (dom.bookingSubmitButton) {
                    dom.bookingSubmitButton.disabled = !isLoggedIn || state.isSubmittingBooking;
                    dom.bookingSubmitButton.classList.toggle('opacity-60', !isLoggedIn);
                    dom.bookingSubmitButton.classList.toggle('cursor-not-allowed', !isLoggedIn);
                }

                if (isLoggedIn && dom.inputName && !dom.inputName.value.trim()) {
                    const userName = user?.user_metadata?.full_name || user?.user_metadata?.name || '';
                    if (userName) {
                        dom.inputName.value = userName;
                    }
                }
            ...
            async function loginForBooking() {
                if (!supabase) {
                    window.alert('Configuração do Supabase ausente. O arquivo app.config.js não foi reconhecido nesta publicação.');
                    return;
                }

                persistBookingProgress();
                ...
                if (error) {
                    ...
                    window.alert(error.message || 'Não foi possível iniciar o login com Google.');
                }
            }

            async function restoreBookingProgress() {
                let raw = null;
                try {
                    raw = localStorage.getItem(STORAGE_KEYS.bookingProgress);
                } catch (error) {
                    console.warn('Não foi possível ler o progresso do agendamento.', error);
                }
                ...
                try {
                    ...
                    if (saved.activeCategory) {
                        updates.activeCategory = saved.activeCategory;
                    }

                    if (saved.selectedServiceId != null) {
                        const service = services.find((item) => String(item.id) === String(saved.selectedServiceId));
                        if (service) {
                    ...
                    if (saved.selectedDateIso) {
                        const selectedDate = availableDates.find((item) => formatDateToIso(item.fullDate) === saved.selectedDateIso);
                        if (selectedDate) {
                            updates.selectedDate = selectedDate;
                        }
                    }
                    ...
                    if (saved.selectedTime && getAvailableTimeSlotsForSelectedDate().includes(saved.selectedTime)) {
                        setState({ selectedTime: saved.selectedTime });
                        renderTimes();
                        syncSummary();
                    }
                    ...
                    console.warn('Não foi possível restaurar o progresso do agendamento.', error);
                }
            }
        ...
        function renderInstagramGallery() {
            ...
            async function submitBooking() {
                ...
                const authenticatedUser = await syncBookingAuthState();
                if (!authenticatedUser) {
                    window.alert('Para confirmar o agendamento, faça login com Google primeiro.');
                    return;
                }
