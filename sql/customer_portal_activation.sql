...
begin
  ...
  if coalesce(v_appointment.attendance_status, '') <> 'completed' then
...
create policy "appointments_update_admin_only" on public.appointments
for update to authenticated
using ((select public.is_admin_user()))
with check ((select public.is_admin_user()));

create policy "service_reviews_select_own_or_admin" on public.service_reviews
for select to authenticated
using (customer_user_id = auth.uid() or (select public.is_admin_user()));
...
create policy "service_reviews_update_admin_only" on public.service_reviews
for update to authenticated
using ((select public.is_admin_user()))
with check ((select public.is_admin_user()));

create policy "service_reviews_delete_admin_only" on public.service_reviews
